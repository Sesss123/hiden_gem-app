...
class DiscoveryRemoteDataSource {
  ...
  Future<List<Map<String, dynamic>>> getAiRecommendationsRaw({
    ...
    final securityHeaders = await VaultService.getSecurityHeaders(
        '/ai/recommendations',
        body: body);
    ...
    if (response.statusCode == 200) {
      try {
        ...
        throw FormatException(
            "Invalid JSON payload from AI recommendations API: $e");
      }
    } else {
      throw Exception(
          "AI Recommendation API failed with status ${response.statusCode}");
    }
  }
}
