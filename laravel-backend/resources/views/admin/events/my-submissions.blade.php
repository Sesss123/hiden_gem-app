...
<div class="space-y-6">
    <div>
        <h2 class="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            <i class="fa-solid fa-file-lines text-emerald-400"></i> My Pending Events
        </h2>
        <p class="text-sm text-slate-400">Events you've created or edited, and their current approval status.</p>
    </div>

    @php $activeStatus = request('status', 'pending'); @endphp
    <div class="glass-card p-4 rounded-2xl flex items-center gap-2">
        <a href="{{ route('admin.events.my-submissions', ['status' => 'pending']) }}"
        ...
        <a href="{{ route('admin.events.my-submissions', ['status' => 'all']) }}"
        ...
        <a href="{{ route('admin.events.my-submissions', ['status' => 'approved']) }}"
        ...
        <a href="{{ route('admin.events.my-submissions', ['status' => 'rejected']) }}"
            class="px-3 py-1.5 rounded-lg text-xs font-semibold {{ $activeStatus === 'rejected' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'text-slate-400 hover:text-white' }}">Rejected</a>
    </div>
                ...
                <tbody class="divide-y divide-slate-800/60 text-sm">
                    ...
                    <tr class="hover:bg-slate-800/40 transition duration-150">
                        ...
                        <td class="py-3 px-6">
                            ...
                            @if($estatus === 'rejected' && $event->review_reason)
                                <div class="text-[11px] text-slate-500 mt-1 max-w-xs">{{ $event->review_reason }}</div>
                            @endif
                        </td>
                        <td class="py-3 px-6 text-slate-400 font-mono text-xs hidden md:table-cell">
                            {{ $event->updated_at ? $event->updated_at->diffForHumans() : 'N/A' }}
                        </td>
                        <td class="py-3 px-6 text-right">
                            <a href="{{ route('admin.events.edit', $event->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-800 hover:bg-emerald-600 text-slate-300 hover:text-white transition shadow" title="Edit">
